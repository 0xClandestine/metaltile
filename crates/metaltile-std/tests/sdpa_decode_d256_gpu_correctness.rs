...
#[test]
fn sdpa_decode_d256_matches_naive_cpu_reference_f32() {
    let n_q_heads = 4usize;
    let n_kv_heads = 1usize; // GQA fan-out 4 (matches Gemma 3 1B layout)
    ...
    let mut kernel = ffai_sdpa_decode_d256::kernel_ir_for(DType::F32);
    ...
    assert!(
        max_diff < 1e-3,
        "sdpa_decode_d256 diverges from CPU reference: max |diff| = {max_diff:.2e} at {max_at} \

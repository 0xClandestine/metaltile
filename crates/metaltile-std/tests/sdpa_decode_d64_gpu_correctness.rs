...
//!
//! Same shape as `sdpa_decode_gpu_correctness.rs` but with head_dim
//! halved to 64. Pins the 2-elt-per-lane Q/K/V layout + the
...
use metaltile_std::ffai::sdpa_decode_d64::ffai_sdpa_decode_d64;
...
#[test]
fn sdpa_decode_d64_matches_naive_cpu_reference_f32() {
    ...
    let mut kernel = ffai_sdpa_decode_d64::kernel_ir_for(DType::F32);
    ...
    assert!(
        max_diff < 1e-4,
        "sdpa_decode_d64 diverges from CPU reference: max |diff| = {max_diff:.2e} at {max_at}",
    );
}

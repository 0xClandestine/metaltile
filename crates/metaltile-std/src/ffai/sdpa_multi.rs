...
pub mod kernel_tests {
    ...
    fn make_setup(
        ...
        TestSetup::new(kernel)
            ...
            .constexpr("causal", causal as u32)
            .constexpr("scale", scale)
            .grid_3d((n_q_heads * n_query) as u32, 1, 1, [1024, 1, 1])
    }

    #[test_kernel(name = "ffai/sdpa_multi/full_f32", dtypes = [f32], tol = 1e-4)]
    fn test_sdpa_multi_full_f32(dt: DType) -> TestSetup {
    ...
    fn test_sdpa_multi_causal_f32(dt: DType) -> TestSetup {
        let (n_q_heads, n_kv_heads, head_dim) = (4usize, 4usize, 128usize);
        let (base_kv, n_query) = (0usize, 8usize);
        let kv_stride = base_kv + n_query;
        let scale = 1.0f32 / (head_dim as f32).sqrt();
    ...
    fn test_sdpa_multi_prefix_gqa_causal_f32(dt: DType) -> TestSetup {
    ...
    fn test_sdpa_multi_full_f16(dt: DType) -> TestSetup {
    ...
    fn test_sdpa_multi_causal_bf16(dt: DType) -> TestSetup {
        let (n_q_heads, n_kv_heads, head_dim) = (4usize, 2usize, 128usize);
        let (base_kv, n_query) = (12usize, 8usize);
        let kv_stride = base_kv + n_query;
        let scale = 1.0f32 / (head_dim as f32).sqrt();

...
mod tests_support {
    ...
    fn sequential_gdn(
        q: &[f32], k: &[f32], v: &[f32], g: &[f32], beta: &[f32], state: &mut [f32],
        t_total: usize, hk: usize, hv: usize, dk: usize, dv: usize,
    ) -> Vec<f32> {
        let hv_per_hk = hv / hk;
    ...
    fn test_gated_delta_wy_chunk(dt: DType) -> TestSetup {
        ...
        let q: Vec<f32> =
            (0..t * hk * dk).map(|i| ((i as f32) * 0.013).sin() * 0.3).collect();
        let k: Vec<f32> =
            (0..t * hk * dk).map(|i| ((i as f32) * 0.017).cos() * 0.3).collect();
        let v: Vec<f32> =
            (0..t * hv * dv).map(|i| ((i as f32) * 0.019).sin() * 0.4).collect();
        let g: Vec<f32> =
            (0..t * hv).map(|i| 0.8 + 0.1 * ((i as f32) * 0.07).cos()).collect();
        let beta: Vec<f32> =
            (0..t * hv).map(|i| 0.3 + 0.1 * ((i as f32) * 0.05).sin()).collect();
...
pub mod tests_support_ctx {
    ...
    fn sequential_gdn(q: &[f32], k: &[f32], v: &[f32], g: &[f32], beta: &[f32], state: &mut [f32], t_total: usize, hk: usize, hv: usize, dk: usize, dv: usize) -> Vec<f32> {
        let hv_per_hk = hv / hk;
        let mut y = vec![0.0_f32; t_total * hv * dv];
        for t in 0..t_total {
            for h_v in 0..hv {
                let h_k = h_v / hv_per_hk;
                let gt = g[t * hv + h_v]; let bt = beta[t * hv + h_v];
                for d_v in 0..dv {
                    let v_val = v[(t * hv + h_v) * dv + d_v];
                    let s_base = (h_v * dv + d_v) * dk;
                    let mut kv_mem = 0.0_f32; let mut decayed = vec![0.0_f32; dk];
                    for s_idx in 0..dk { let s = state[s_base+s_idx]*gt; decayed[s_idx]=s; kv_mem += s*k[(t*hk+h_k)*dk+s_idx]; }
                    let delta = (v_val - kv_mem) * bt;
                    let mut out = 0.0_f32;
                    for s_idx in 0..dk { let s_new = decayed[s_idx]+k[(t*hk+h_k)*dk+s_idx]*delta; state[s_base+s_idx]=s_new; out += s_new*q[(t*hk+h_k)*dk+s_idx]; }
                    y[(t * hv + h_v) * dv + d_v] = out;
                }
            }
        }
    ...
    fn run_gated_delta_wy_chunk(q: &[f32], k: &[f32], v: &[f32], g: &[f32], beta: &[f32], state_in: &[f32], dt: Dt, b: usize, t_total: usize, hk: usize, hv: usize, dk: usize, dv: usize, c: usize) -> (Vec<f32>, Vec<f32>) {
        ...
        let result = ctx.dispatch_with_grid(&kernel, &buffers, &BTreeMap::new(), [1, n_total, 1], [32, 1, 1]).expect("mt_gated_delta_wy_chunk dispatch");
    ...
    fn make_inputs(seed_phase: f32, t: usize, hk: usize, hv: usize, dk: usize, dv: usize) -> (Vec<f32>,Vec<f32>,Vec<f32>,Vec<f32>,Vec<f32>,Vec<f32>) {
        let n_total = t * hv;
        let kscale = (2.0_f32 / dk as f32).sqrt();
        let q: Vec<f32> = (0..t*hk*dk).map(|i| ((i as f32+seed_phase)*0.0173).sin()*kscale).collect();
        let k: Vec<f32> = (0..t*hk*dk).map(|i| ((i as f32+seed_phase)*0.0211).cos()*kscale).collect();
        let v: Vec<f32> = (0..t*n_total*dv).map(|i| ((i as f32+seed_phase)*0.029).sin()*0.3).collect();
        let g: Vec<f32> = (0..t*n_total).map(|i| 0.85+0.1*((i as f32+seed_phase)*0.013).sin()).collect();
        let beta: Vec<f32> = (0..t*n_total).map(|i| 0.5+0.2*((i as f32+seed_phase)*0.017).cos()).collect();
    ...
    fn wy_chunk_identity_at_g1_beta0_f32() {
        ...
        let n_total = b * hv; let kscale = (2.0_f32 / dk as f32).sqrt();
        ...
        let v = vec![0.0_f32; t*n_total*dv]; let g = vec![1.0_f32; t*n_total]; let beta = vec![0.0_f32; t*n_total];
        ...
        let (y_wy, s_wy) = run_gated_delta_wy_chunk(&q,&k,&v,&g,&beta,&state_in,Dt::F32,b,t,hk,hv,dk,dv,c);
    ...
    fn wy_chunk_dk_equals_dv_square_state_f32() {
        ...
        let max_y = y_seq.iter().zip(&y_wy).map(|(a,b)|(a-b).abs()).fold(0.0_f32,f32::max);
        let max_s = s_seq.iter().zip(&s_wy).map(|(a,b)|(a-b).abs()).fold(0.0_f32,f32::max);
        assert!(max_y < 5e-3, "square-state y = {max_y:.2e}"); assert!(max_s < 5e-3, "square-state s = {max_s:.2e}");
    }
    ...
    fn wy_chunk_two_chunk_chain_f32() {
        ...
        let max_y = y_seq.iter().zip(&y_wy).map(|(a,b)|(a-b).abs()).fold(0.0_f32,f32::max);
        let max_s = s_seq.iter().zip(&s_wy).map(|(a,b)|(a-b).abs()).fold(0.0_f32,f32::max);
        assert!(max_y < 5e-3, "two-chunk y = {max_y:.2e}"); assert!(max_s < 5e-3, "two-chunk s = {max_s:.2e}");
    }
    ...
    fn wy_chunk_aggressive_decay_f32() {
        ...
        let max_y = y_seq.iter().zip(&y_wy).map(|(a,b)|(a-b).abs()).fold(0.0_f32,f32::max);
        let max_s = s_seq.iter().zip(&s_wy).map(|(a,b)|(a-b).abs()).fold(0.0_f32,f32::max);
        assert!(max_y < 5e-3, "aggressive-decay y = {max_y:.2e}"); assert!(max_s < 5e-3, "aggressive-decay s = {max_s:.2e}");
    }
    ...
    fn wy_chunk_matches_oracle_multi_chunk_f16() {
        ...
        let mut s_seq = state_in.clone();
        let y_seq = sequential_gdn(&q,&k,&v,&g,&beta,&mut s_seq,t,hk,hv,dk,dv);
        let (y_wy, s_wy) = run_gated_delta_wy_chunk(&q,&k,&v,&g,&beta,&state_in,Dt::F16,b,t,hk,hv,dk,dv,c);
        let max_y = y_seq.iter().zip(&y_wy).map(|(a,b)|(a-b).abs()).fold(0.0_f32,f32::max);
        let max_s = s_seq.iter().zip(&s_wy).map(|(a,b)|(a-b).abs()).fold(0.0_f32,f32::max);
        assert!(max_y < 5e-2, "f16 y = {max_y:.2e}"); assert!(max_s < 5e-2, "f16 s = {max_s:.2e}");
    }
    ...
    fn wy_chunk_matches_oracle_multi_chunk_bf16() {
        ...
        let mut s_seq = state_in.clone();
        let y_seq = sequential_gdn(&q,&k,&v,&g,&beta,&mut s_seq,t,hk,hv,dk,dv);
        let (y_wy, s_wy) = run_gated_delta_wy_chunk(&q,&k,&v,&g,&beta,&state_in,Dt::Bf16,b,t,hk,hv,dk,dv,c);
        let max_y = y_seq.iter().zip(&y_wy).map(|(a,b)|(a-b).abs()).fold(0.0_f32,f32::max);
        let max_s = s_seq.iter().zip(&s_wy).map(|(a,b)|(a-b).abs()).fold(0.0_f32,f32::max);
        assert!(max_y < 2e-1, "bf16 y = {max_y:.2e}"); assert!(max_s < 2e-1, "bf16 s = {max_s:.2e}");
    }
}

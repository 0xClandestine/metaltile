...
mod tests_support {
    ...
    fn oracle(
        ...
        for r in 0..hv {
            ...
            for d in 0..dv {
                ...
                inner[base + d] = y[base + d] * inv_rms * norm_w[d] * g;
            }
        }
    ...
    fn make_setup(dt: DType, hv: usize, dv: usize, out_dim: usize, group_size: usize) -> TestSetup {
        ...
        for r in 0..out_dim {
            let row = source(in_dim, (10 + r) as u64, 1.0, 0.0);
            let (rp, rs, rb) = quantize_int4_row(&row, group_size);
            let stride_u32 = in_dim / 8;
            w_packed[r * stride_u32..(r + 1) * stride_u32].copy_from_slice(&rp);

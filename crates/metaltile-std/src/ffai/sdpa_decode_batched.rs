...
mod tests_support {
    ...
    /// Interleave K independent Q-vectors `[n_q_heads, head_dim]` into
    /// `[n_q_heads, K, head_dim]` — the batched kernel's input layout.
    fn interleave_qs(qs: &[&[f32]], n_q_heads: usize, head_dim: usize) -> Vec<f32> {
        let k = qs.len();
    ...
    fn interleave_expected(
        ...
        for h in 0..n_q_heads {
            for e in expected {
                out.extend_from_slice(&e[h * head_dim..(h + 1) * head_dim]);
            }
        }
        out
    }
    ...
    fn test_sdpa_decode_batched_q2_dense(dt: DType) -> TestSetup {
        ...
        let exp_a = naive_sdpa(&q_a, &k, &v, n_q_heads, n_kv_heads, head_dim, n_kv, scale);
        let exp_b = naive_sdpa(&q_b, &k, &v, n_q_heads, n_kv_heads, head_dim, n_kv, scale);
        let q_batched = interleave_qs(&[&q_a, &q_b], n_q_heads, head_dim);
        let expected = interleave_expected(&[&exp_a, &exp_b], n_q_heads, head_dim);
        let mut kernel = sdpa_decode_batched_q2::kernel_ir_for(dt);
        kernel.mode = KernelMode::Reduction;
        TestSetup::new(kernel)
            .input(TestBuffer::from_vec("q", pack(&q_batched, dt), dt))

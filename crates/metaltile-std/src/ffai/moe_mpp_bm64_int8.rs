...
pub mod kernel_tests {
    ...
    use super::{super::moe::mt_moe_gather_qmm_b8, mt_moe_gather_qmm_mma_int8_bm64_mpp};
    ...
    fn make_setup(
        ...
        let weight_unpacked: Vec<u32> =
            (0..total_weights).map(|i| ((i as u32).wrapping_mul(w_seed) + 3) & 0xff).collect();
        ...
        let x: Vec<f32> = (0..t_rows * k_in).map(|i| 0.05 * (i as f32 * x_phase).sin()).collect();
        ...
        let mut kernel = mt_moe_gather_qmm_mma_int8_bm64_mpp::kernel_ir_for(dt);
    ...
    #[test_kernel(name = "ffai/moe_mpp_bm64_int8/vs_b8_f32_clean", dtypes = [f32], tol = 1e-2)]
    fn test_moe_mpp_bm64_int8_clean_tile(dt: DType) -> TestSetup {
        make_setup(4, 64, 64, 32, 64, 7, 0.013, dt)
    }

    #[test_kernel(name = "ffai/moe_mpp_bm64_int8/vs_b8_f32_multi", dtypes = [f32], tol = 1e-2)]
    fn test_moe_mpp_bm64_int8_multi_tile(dt: DType) -> TestSetup {
        make_setup(8, 128, 128, 64, 128, 11, 0.017, dt)
    }

    #[test_kernel(name = "ffai/moe_mpp_bm64_int8/vs_b8_bf16_clean", dtypes = [bf16], tol = 1e-2)]
    fn test_moe_mpp_bm64_int8_bf16_clean(dt: DType) -> TestSetup {
        make_setup(4, 64, 64, 32, 64, 7, 0.013, dt)
    }
}

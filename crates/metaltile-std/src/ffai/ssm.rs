...
mod tests_support {
    ...
    /// Round f32 values through the target dtype and back (for oracle alignment).
    ...
    #[test_kernel(name = "ffai/ssm/conv1d_causal_step", dtypes = [f32, f16, bf16], tol = 5e-2)]
    fn test_conv1d_causal_step(dt: DType) -> TestSetup {
        ...
        TestSetup::new(kernel_ir)
            ...
            .expect(TestBuffer::from_vec("y", pack(&expected_y, dt), dt))
            .expect(TestBuffer::from_vec("state", pack(&state_oracle, dt), dt))
    ...
    /// CPU oracle: x=0 invariant — with x=0 the recurrence reduces to h_new = decay*h_old.
    #[test_kernel(name = "ffai/ssm/step_decay_invariant", dtypes = [f32], tol = 1e-5)]
    fn test_ssm_step_decay_invariant(dt: DType) -> TestSetup {
        use super::ssm_step;
        let n_heads = 2usize;
        let head_dim = 8usize;
    ...
    /// CPU oracle for mt_ssm_step (MLX-aligned, simd_sum across state dim).
    ...
    #[test_kernel(name = "ffai/ssm/mt_step", dtypes = [f32], tol = 5e-5)]
    fn test_mt_ssm_step(dt: DType) -> TestSetup {
        use super::mt_ssm_step;
        let n_heads = 4usize;
        ...
        TestSetup::new(kernel_ir)
            .input(TestBuffer::from_vec("x", pack(&x, dt), dt))
            .input(TestBuffer::from_vec("a_log", pack(&a_log, dt), dt))
            .input(TestBuffer::from_vec("b_mat", pack(&b_mat, dt), dt))
            .input(TestBuffer::from_vec("c_mat", pack(&c_mat, dt), dt))
            .input(TestBuffer::from_vec("d_skip", pack(&d_skip, dt), dt))
            .input(TestBuffer::from_vec("dt", pack(&dt_in, dt), dt))
            .input(TestBuffer::from_vec("state_in", pack(&state_in, dt), dt))
            .input(TestBuffer::from_vec("dh", u32_le(dh as u32), DType::U32))
            .input(TestBuffer::from_vec("ds", u32_le(ds as u32), DType::U32))
            .input(TestBuffer::from_vec("n_heads", u32_le(n_heads as u32), DType::U32))
            .input(TestBuffer::from_vec(
                "heads_per_group",
                u32_le(heads_per_group as u32),
                DType::U32,
            ))
            .expect(TestBuffer::from_vec("state_out", pack(&state_expected, dt), dt))
            .expect(TestBuffer::from_vec("out", pack(&out_expected, dt), dt))
            .grid_3d(dh as u32, n_total as u32, 1, [32, 1, 1])
    }

    /// Exercises n_per_t = 4 (ds=128) — the multi-iter inner-loop index path.
    #[test_kernel(name = "ffai/ssm/mt_step_ds128", dtypes = [f32], tol = 1e-4)]
    fn test_mt_ssm_step_ds_128(dt: DType) -> TestSetup {
        use super::mt_ssm_step;
        let n_heads = 4usize;
        let heads_per_group = 2usize;
        let batch = 1usize;
    ...
    /// bf16 path: wider tolerance (7-bit mantissa + simd_sum over 32 lanes).
    #[test_kernel(name = "ffai/ssm/mt_step_bf16", dtypes = [bf16], tol = 1e-1)]
    fn test_mt_ssm_step_bf16(dt: DType) -> TestSetup {
        use super::mt_ssm_step;
        let n_heads = 4usize;
        let heads_per_group = 2usize;
        let batch = 1usize;

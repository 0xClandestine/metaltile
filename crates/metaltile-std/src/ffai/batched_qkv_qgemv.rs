...
use metaltile::test_kernel;
    ...
    fn test_batched_qkv_qgemv_f32_gqa(dt: DType) -> TestSetup {
        ...
        let out_v = out_k;
        let x: Vec<f32> =
            source(in_dim, 0x11, 2.0, 0.05).iter().map(|&v| round_dt(v, dt)).collect();
        ...
        let mut expected =
            naive_gemv(&wq_p, &round(&sq), &round(&bq), &x, in_dim, group_size, out_q);
        ...
        TestSetup::new(kernel)
            ...
            .input(TestBuffer::from_vec(
                "out",
                pack_f32_dt(&vec![0.0f32; out_q + out_k + out_v], dt),
                dt,
            ))

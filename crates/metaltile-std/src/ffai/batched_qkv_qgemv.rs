...
mod tests_support {
    ...
    fn naive_gemv(weight: &[u32], scales: &[f32], biases: &[f32], x: &[f32], in_dim: usize, group_size: usize, out_dim: usize) -> Vec<f32> {
        ...
        (0..out_dim).map(|row| {
            ...
            for (d, &x_d) in x.iter().enumerate().take(in_dim) {
                ...
                acc += (q as f32 * rs[d / group_size] + rb[d / group_size]) * x_d;
            }
            acc
        }).collect()
    }
    ...
    #[test_kernel(name = "ffai/batched_qkv_qgemv/f32_gqa", dtypes = [f32], tol = 5e-3)]
    fn test_batched_qkv_qgemv_f32_gqa(dt: DType) -> TestSetup {
        let (in_dim, group_size, out_q, out_k) = (256usize, 64usize, 16usize, 4usize);
        let out_v = out_k;
        let x: Vec<f32> = source(in_dim, 0x11, 2.0, 0.05).iter().map(|&v| round_dt(v, dt)).collect();
        ...
        let kernel = ffai_batched_qkv_qgemv::kernel_ir_for(dt);
        TestSetup::new(kernel)
            .input(TestBuffer::from_vec("x", pack_f32_dt(&x, dt), dt))
            .input(TestBuffer::from_vec("w_q", pack_u32_bytes(&wq_p), DType::U32))
            .input(TestBuffer::from_vec("scales_q", pack_f32_dt(&sq, dt), dt))
            .input(TestBuffer::from_vec("biases_q", pack_f32_dt(&bq, dt), dt))
            .input(TestBuffer::from_vec("w_k", pack_u32_bytes(&wk_p), DType::U32))
            .input(TestBuffer::from_vec("scales_k", pack_f32_dt(&sk, dt), dt))
            .input(TestBuffer::from_vec("biases_k", pack_f32_dt(&bk, dt), dt))
            .input(TestBuffer::from_vec("w_v", pack_u32_bytes(&wv_p), DType::U32))
            .input(TestBuffer::from_vec("scales_v", pack_f32_dt(&sv, dt), dt))
            .input(TestBuffer::from_vec("biases_v", pack_f32_dt(&bv, dt), dt))
            .input(TestBuffer::from_vec("out", pack_f32_dt(&vec![0.0f32; out_q + out_k + out_v], dt), dt))
            .input(TestBuffer::from_vec("out_q", (out_q as u32).to_le_bytes().to_vec(), DType::U32))
            .input(TestBuffer::from_vec("out_k", (out_k as u32).to_le_bytes().to_vec(), DType::U32))
            .input(TestBuffer::from_vec("out_v", (out_v as u32).to_le_bytes().to_vec(), DType::U32))

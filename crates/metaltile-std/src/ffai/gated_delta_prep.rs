...
pub mod tests_support_ctx {
    ...
    fn cpu_prep(
        ...
        for batch in 0..b {
            ...
            for hk_idx in 0..hk {
                ...
                for d in 0..dk {
                    q_ssq += conv_out[q_base + row_off + d] * conv_out[q_base + row_off + d];
                    k_ssq += conv_out[k_base + row_off + d] * conv_out[k_base + row_off + d];
                }
            ...
            for hv_idx in 0..hv {
                ...
                let dt_v = softplus_unclamped(a_raw[n] + dt_bias[hv_idx]);
    ...
    fn make_fixture(
        ...
        let q_norm_weight = if identity {
            vec![ws; hk * dk]
        } else {
            (0..hk * dk).map(|i| ws * (1.0 + ((i % 11) as f32) * 0.05)).collect()
        };
        let k_norm_weight = if identity {
            vec![ws; hk * dk]
        } else {
            (0..hk * dk).map(|i| ws * (1.0 + ((i % 13) as f32) * 0.04)).collect()
        };
        let state_in = (0..b * hv * dv * dk)
            .map(|i| (((i + seed_offset) as f32) * 0.0073).cos() * 0.1)
            .collect();
    ...
    fn prep_step_f32_multi_step_8_consecutive() {
        ...
        for step in 0..n_steps {
            ...
            let f = Fixture {
                conv_out: conv_out.clone(),
                a_log: a_log.clone(),
                dt_bias: dt_bias.clone(),
                a_raw: a_raw.clone(),
                b_raw: b_raw.clone(),
                q_norm_weight: weights_q.clone(),
                k_norm_weight: weights_k.clone(),
                state_in: state_gpu.clone(),
            };

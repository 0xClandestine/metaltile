...
pub mod kernel_tests {
    ...
    use super::{super::moe::mt_moe_gather_qmm_b8, mt_moe_gather_qmm_mma_int8_bm8_mpp};
    ...
    fn pack_int8_row(weights: &[u32]) -> Vec<u32> {
        assert!(weights.len().is_multiple_of(4));
        weights
            .chunks_exact(4)
            .map(|chunk| {
                let mut packed = 0u32;
    ...
    fn make_setup(
        ...
        let weight_unpacked: Vec<u32> = (0..total_weights)
            .map(|i| ((i as u32).wrapping_mul(13).wrapping_add(7)) & 0xff)
            .collect();
        ...
        let out_size_f32 = t_rows * n_out * 4;
        let mut ref_kernel = mt_moe_gather_qmm_b8::kernel_ir_for(DType::F32);
        ref_kernel.mode = KernelMode::Reduction;
        let ref_setup = TestSetup::new(ref_kernel)
            .input(TestBuffer::from_vec("x", pack_bytes(&x, DType::F32), DType::F32))
        ...
        let out_size = t_rows * n_out * dt.size_bytes();
        let mut kernel = mt_moe_gather_qmm_mma_int8_bm8_mpp::kernel_ir_for(dt);
        kernel.mode = KernelMode::Reduction;
        TestSetup::new(kernel)
            .input(TestBuffer::from_vec("x", pack_bytes(&x, dt), dt))
            .input(TestBuffer::from_vec("w", pack_u32(&weight_packed), DType::U32))
            .input(TestBuffer::from_vec("scales", pack_bytes(&scales, dt), dt))
            .input(TestBuffer::from_vec("biases", pack_bytes(&biases, dt), dt))
            ...
            .constexpr("k_in", k_in as u32)
            .constexpr("group_size", group_size as u32)
            .grid_3d(n_out.div_ceil(32) as u32, t_rows.div_ceil(8) as u32, 1, [32, 1, 1])
            .compare_against(ref_setup)
    }

    fn uniform_indices(n_experts: usize, t_rows: usize) -> Vec<u32> {
        (0..t_rows).map(|r| ((r * n_experts) / t_rows) as u32).collect()
    }

    #[test_kernel(name = "ffai/moe_mpp_bm8_int8/vs_b8_f32_small", dtypes = [f32], tol = 1e-2)]
    fn test_moe_mpp_bm8_int8_f32_small(dt: DType) -> TestSetup {
        let idx = uniform_indices(4, 8);
        make_setup(4, 8, 64, 64, 32, &idx, dt)
    }

    #[test_kernel(name = "ffai/moe_mpp_bm8_int8/vs_b8_f16_small", dtypes = [f16], tol = 1e-2)]
    fn test_moe_mpp_bm8_int8_f16_small(dt: DType) -> TestSetup {
        let idx = uniform_indices(4, 8);
        make_setup(4, 8, 64, 64, 32, &idx, dt)
    }

    #[test_kernel(name = "ffai/moe_mpp_bm8_int8/vs_b8_bf16_small", dtypes = [bf16], tol = 1e-2)]
    fn test_moe_mpp_bm8_int8_bf16_small(dt: DType) -> TestSetup {
        let idx = uniform_indices(4, 8);
        make_setup(4, 8, 64, 64, 32, &idx, dt)
    }

    #[test_kernel(name = "ffai/moe_mpp_bm8_int8/vs_b8_f16_multi", dtypes = [f16], tol = 1e-2)]
    fn test_moe_mpp_bm8_int8_f16_multi(dt: DType) -> TestSetup {
        let idx = uniform_indices(8, 16);
        make_setup(8, 16, 128, 128, 64, &idx, dt)
    }

    #[test_kernel(name = "ffai/moe_mpp_bm8_int8/vs_b8_f16_ragged", dtypes = [f16], tol = 1e-2)]
    fn test_moe_mpp_bm8_int8_f16_ragged(dt: DType) -> TestSetup {
        make_setup(3, 5, 64, 64, 32, &[0, 0, 1, 1, 2], dt)
    }

    #[test_kernel(name = "ffai/moe_mpp_bm8_int8/vs_b8_f16_prod", dtypes = [f16], tol = 1e-2)]
    fn test_moe_mpp_bm8_int8_f16_prod(dt: DType) -> TestSetup {
        make_setup(128, 8, 512, 2048, 64, &[3, 17, 42, 55, 71, 88, 99, 120], dt)
    }

    #[test_kernel(name = "ffai/moe_mpp_bm8_int8/vs_b8_bf16_prod", dtypes = [bf16], tol = 1e-2)]
    fn test_moe_mpp_bm8_int8_bf16_prod(dt: DType) -> TestSetup {
        make_setup(128, 8, 512, 2048, 64, &[3, 17, 42, 55, 71, 88, 99, 120], dt)
    }
}

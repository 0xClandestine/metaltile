...
//! Fused image resize (**bicubic**) + per-channel normalize + interleaved→
//! NCHW — the higher-quality sibling of [`super::ffai_resize_normalize`]
//! (which is bilinear).
//!
//! Same contract as the bilinear kernel: take an interleaved `[src_h, src_w,
//! 3]` source image, resample to `target_h × target_w`, normalize each
//! channel `(v − mean[c]) / std[c]`, and write the planar NCHW `[3, target_h,
//! target_w]` tensor — all in one dispatch. The only difference is the
//! sampler: a **4×4 cubic-convolution** (Keys, `a = −0.75`) instead of the
//! 2×2 bilinear, which preserves edges far better when **down-scaling** a
//! large photo to a tower's native grid (the bilinear kernel aliases there).
//!
//! Convention matches `torch.nn.functional.interpolate(mode="bicubic",
//! align_corners=False)`: half-pixel source centers
//! `src = (out + 0.5)·scale − 0.5`, the 4 taps at `floor(src) − 1 …
//! floor(src) + 2`, cubic weights from the fractional offset, and each tap's
//! pixel coordinate **clamped** into `[0, src_dim − 1]` (edge replicate).
//! The Keys cubic weights sum to 1 (partition of unity), so no renormalize.
//!
//! `src_w/src_h/target_w/target_h` are 1-element `u32` buffers (runtime
//! scalars) so ONE compiled kernel serves every image size. `mean`/`std` are
//! `[3]` f32.
//!
//! ## DISPATCH INVARIANTS
...
pub mod kernel_tests {
    ...
    use super::ffai_resize_normalize_bicubic;
    ...
    // Up-size too (exercises the other scale direction).
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [2e-4, 5e-3, 3e-2])]
...
/// New-syntax bench at a representative VL preprocess shape (≈640×480
/// source → 448×448), matching the bilinear kernel's bench.

...
pub mod tests_support {
    ...
    use super::super::moe::mt_moe_gather_qmm_int4;
    ...
    fn pack_bytes_bf16(vals: &[f32]) -> Vec<u8> { vals.iter().flat_map(|v| half::bf16::from_f32(*v).to_le_bytes()).collect() }
    fn pack_bytes(vals: &[f32], dt: Dt) -> Vec<u8> {
        match dt { Dt::F32 => pack_bytes_f32(vals), Dt::Bf16 => pack_bytes_bf16(vals) }
    }
    ...
    fn pack_int4_row(weights: &[u32]) -> Vec<u32> {
        ...
        weights.chunks_exact(8).map(|chunk| {
            let mut packed = 0u32;
            for (i, &q) in chunk.iter().enumerate() { packed |= (q & 0xf) << (i * 4); }
            packed
        }).collect()
    }
    ...
    fn run_m1(weight_packed: &[u32], scales: &[f32], biases: &[f32], x: &[f32], expert_offsets: &[u32], n_out: usize, t_rows: usize, k_in: usize, group_size: usize, n_experts: usize) -> Vec<f32> {
    ...
    fn run_bm64(weight_packed: &[u32], scales: &[f32], biases: &[f32], x: &[f32], indices: &[u32], n_out: usize, t_rows: usize, k_in: usize, group_size: usize, dt: Dt) -> Vec<f32> {
        let mut b: BTreeMap<String, Vec<u8>> = BTreeMap::new();
    ...
    fn moe_gather_qmm_mma_int4_bm64_mpp_matches_m1_clean_tile() {
        ...
        let y_m1 = run_m1(&weight_packed, &scales, &biases, &x, &expert_offsets, n_out, t_rows, k_in, group_size, n_experts);
        let y_mpp = run_bm64(&weight_packed, &scales, &biases, &x, &indices, n_out, t_rows, k_in, group_size, Dt::F32);
    ...
    fn moe_gather_qmm_mma_int4_bm64_mpp_matches_m1_multi_tile() {
        ...
        let indices: Vec<u32> = (0..t_rows).map(|r| (r / (t_rows / n_experts)) as u32).collect();
        let total_weights = n_experts * n_out * k_in;
        let weight_unpacked: Vec<u32> = (0..total_weights).map(|i| ((i as u32) * 11 + 5) & 0xf).collect();
        let weight_packed: Vec<u32> = weight_unpacked.chunks_exact(k_in).flat_map(pack_int4_row).collect();
        let groups_total = n_experts * n_out * (k_in / group_size);
    ...
    fn moe_gather_qmm_mma_int4_bm64_mpp_bf16_matches_m1_clean_tile() {
        ...
        expert_offsets[n_experts] = t_rows as u32;
        let y_m1 = run_m1(&weight_packed, &scales, &biases, &x, &expert_offsets, n_out, t_rows, k_in, group_size, n_experts);
        let y_mpp = run_bm64(&weight_packed, &scales, &biases, &x, &indices, n_out, t_rows, k_in, group_size, Dt::Bf16);
        let (cos, nan_count) = cosine_f64(&y_m1, &y_mpp);

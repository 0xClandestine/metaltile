...
pub mod kernel_tests {
    ...
    fn make_shared_data(
        ...
        w_seed: u32,
        x_seed: f32,
    ) -> (Vec<f32>, Vec<u32>, Vec<f32>, Vec<f32>, Vec<u32>, Vec<u32>) {
        ...
        let weight_unpacked: Vec<u32> =
            (0..total_weights).map(|i| ((i as u32).wrapping_mul(w_seed) + 5) & 0xf).collect();
        ...
        let x: Vec<f32> = (0..t_rows * k_in).map(|i| x_seed * (i as f32 * 0.013).sin()).collect();
        ...
        (x, weight_packed, scales, biases, indices, expert_offsets)
    }
    ...
    fn make_setup(
        ...
        let (x, weight_packed, scales, biases, indices, expert_offsets) =
            make_shared_data(n_experts, k_in, n_out, group_size, t_rows, w_seed, x_seed);
        ...
        let mut kernel = mt_moe_gather_qmm_mma_int4_bm64_mpp::kernel_ir_for(dt);
        ...
        TestSetup::new(kernel)
            ...
            .constexpr("k_in", k_in as u32)
            .constexpr("group_size", group_size as u32)
            .grid_3d(n_out.div_ceil(64) as u32, t_rows.div_ceil(64) as u32, 1, [128, 1, 1])
            .compare_against(ref_setup)
    }

    #[test_kernel(name = "ffai/moe_mpp_bm64/vs_m1_f32_clean", dtypes = [f32], tol = 1e-2)]
    fn test_moe_mpp_bm64_clean_tile(dt: DType) -> TestSetup {
        make_setup(4, 64, 64, 32, 64, 7, 0.05, dt)
    }

    #[test_kernel(name = "ffai/moe_mpp_bm64/vs_m1_f32_multi", dtypes = [f32], tol = 1e-2)]
    fn test_moe_mpp_bm64_multi_tile(dt: DType) -> TestSetup {
        make_setup(8, 128, 128, 64, 128, 11, 0.05, dt)
    }

    #[test_kernel(name = "ffai/moe_mpp_bm64/vs_m1_bf16_clean", dtypes = [bf16], tol = 1e-2)]
    fn test_moe_mpp_bm64_bf16_clean(dt: DType) -> TestSetup {
        make_setup(4, 64, 64, 32, 64, 7, 0.05, dt)
    }
}

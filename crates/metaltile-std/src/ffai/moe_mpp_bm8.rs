...
pub mod tests_support {
    ...
    fn run_case(case: &Case) {
        ...
        let weight_unpacked: Vec<u32> = (0..total_weights).map(|i| ((i as u32).wrapping_mul(13).wrapping_add(7)) & 0xf).collect();
        ...
        let y_mpp: Vec<f32> = {
            ...
            b.insert("group_size".into(), (group_size as u32).to_le_bytes().to_vec());
            let ctx = Context::new().unwrap();
            let mut k = mt_moe_gather_qmm_mma_int4_bm8_mpp::kernel_ir_for(dt.to_dtype());
            k.mode = KernelMode::Reduction;
        ...
        assert!(cos >= case.min_cos, "[{}] MPP MoE BM=8 vs m1 cosine = {:.6} (want ≥ {:.3})", case.label, cos, case.min_cos);
    }

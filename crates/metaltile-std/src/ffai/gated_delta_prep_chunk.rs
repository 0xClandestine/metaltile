...
pub mod tests_support {
    ...
    struct Fixture {
        conv_out: Vec<f32>, a_log: Vec<f32>, dt_bias: Vec<f32>,
        a_raw: Vec<f32>, b_raw: Vec<f32>,
        q_norm_weight: Vec<f32>, k_norm_weight: Vec<f32>, state_in: Vec<f32>,
    }

    fn make_fixture(b: usize, t: usize, hv: usize, hk: usize, dv: usize, dk: usize, identity: bool, ws: f32) -> Fixture {
        let stride_b = 2 * hk * dk + hv * dv;
        let conv_out = (0..b*t*stride_b).map(|i| ((i as f32)*0.0131).sin()*0.1).collect();
        let a_log = (0..hv).map(|i| -2.0-(i as f32)*0.05).collect();
        let dt_bias = (0..hv).map(|i| -0.3+(i as f32)*0.02).collect();
        let a_raw = (0..b*t*hv).map(|i| -0.2+((i as f32)*0.04).sin()*0.2).collect();
        let b_raw = (0..b*t*hv).map(|i| -0.2+((i as f32)*0.03).cos()*0.2).collect();
        ...
        let state_in = (0..b*hv*dv*dk).map(|i| ((i as f32)*0.0073).cos()*0.02).collect();
    ...
    fn run_gpu(f: &Fixture, dt: Dt, b: usize, t: usize, hv: usize, hk: usize, dv: usize, dk: usize) -> (Vec<f32>, Vec<f32>) {
    ...
    fn run_cell(b: usize, t: usize, hv: usize, hk: usize, dv: usize, dk: usize, dt: Dt, identity: bool, ws: f32) -> (f32, f32) {
        let _g = gpu_lock();
        let raw = make_fixture(b, t, hv, hk, dv, dk, identity, ws);
        let f = round_fixture(&raw, dt);
        let (y_cpu, state_cpu) = cpu_chunk_oracle(&f.conv_out, &f.a_log, &f.dt_bias, &f.a_raw, &f.b_raw, &f.q_norm_weight, &f.k_norm_weight, &f.state_in, b, t, hv, hk, dv, dk);
        let (y_gpu, state_gpu) = run_gpu(&f, dt, b, t, hv, hk, dv, dk);
        (cosine(&y_gpu, &y_cpu), cosine(&state_gpu, &state_cpu))
    }

    #[test] fn prep_chunk_f32_qwen36_t1() {
    ...
    #[test] fn prep_chunk_f32_qwen36_t8_identity() {
    ...
    #[test] fn prep_chunk_f32_qwen36_t8_weighted() {
    ...
    #[test] fn prep_chunk_f32_long_t32_small_shape() {
    ...
    #[test] fn prep_chunk_bf16_small_shape_t32() {

...
pub fn mt_moe_gather_qmm_int4_m8<T>(
    ...
    let x_row_base = row * k_in;
    // 8 accumulator slots — one per m-cell in the chunk.
    stack_alloc("accs", 8, "f32");
    for _m in range(0u32, 8u32, 1u32) {
        stack_store("accs", _m, 0.0f32);
    }
    for pack_idx in range(lane, total_packs, 32u32) {
        let k_first = pack_idx * 8u32;
        let g = k_first / group_size;
        ...
        let x6 = load(x[x_row_base + k_first + 6u32]).cast::<f32>();
        let x7 = load(x[x_row_base + k_first + 7u32]).cast::<f32>();
        // 8 m-cells — DSL unrolls range(0,8) at codegen.
        for _m in range(0u32, 8u32, 1u32) {
            ...
            let dot = ((p >> 0u32) & 15u32).cast::<f32>() * s * x0
                + b * x0
                + ((p >> 4u32) & 15u32).cast::<f32>() * s * x1
                + b * x1
                + ((p >> 8u32) & 15u32).cast::<f32>() * s * x2
                + b * x2
                + ((p >> 12u32) & 15u32).cast::<f32>() * s * x3
                + b * x3
                + ((p >> 16u32) & 15u32).cast::<f32>() * s * x4
                + b * x4
                + ((p >> 20u32) & 15u32).cast::<f32>() * s * x5
                + b * x5
                + ((p >> 24u32) & 15u32).cast::<f32>() * s * x6
                + b * x6
                + ((p >> 28u32) & 15u32).cast::<f32>() * s * x7
                + b * x7;
            let prev = stack_load("accs", _m);
            stack_store("accs", _m, prev + dot);
        }
    }
    // Cross-lane reduce + store.
    for _m in range(0u32, 8u32, 1u32) {
        let r = simd_sum(stack_load("accs", _m));
        if lane == 0u32 {
            store(out[row * m_out + m_base + _m], r.cast::<T>());
        }
    }
}
...
pub fn mt_moe_gather_qmm_int4_m16<T>(
    ...
    let scale_expert_base = expert * m_out * groups_per_row;
    let x_row_base = row * k_in;
    // 16 accumulator slots in two 8-element banks.
    stack_alloc("accs_lo", 8, "f32");
    stack_alloc("accs_hi", 8, "f32");
    for _m in range(0u32, 8u32, 1u32) {
        stack_store("accs_lo", _m, 0.0f32);
        stack_store("accs_hi", _m, 0.0f32);
    }
    for pack_idx in range(lane, total_packs, 32u32) {
        let k_first = pack_idx * 8u32;
        let g = k_first / group_size;
        ...
        let x6 = load(x[x_row_base + k_first + 6u32]).cast::<f32>();
        let x7 = load(x[x_row_base + k_first + 7u32]).cast::<f32>();
        // Low 8 m-cells (0..7) — DSL unrolls range(0,8) at codegen.
        for _m in range(0u32, 8u32, 1u32) {
            let wrb = weight_expert_base + (m_base + _m) * total_packs;
            let srb = scale_expert_base + (m_base + _m) * groups_per_row;
            let p = load(weight_packed[wrb + pack_idx]);
            ...
            let dot = ((p >> 0u32) & 15u32).cast::<f32>() * s * x0
                ...
                + ((p >> 28u32) & 15u32).cast::<f32>() * s * x7
                + b * x7;
            let prev = stack_load("accs_lo", _m);
            stack_store("accs_lo", _m, prev + dot);
        }
        // High 8 m-cells (8..15) — same structure, offset base by 8.
        for _m in range(0u32, 8u32, 1u32) {
            let wrb = weight_expert_base + (m_base + 8u32 + _m) * total_packs;
            let srb = scale_expert_base + (m_base + 8u32 + _m) * groups_per_row;
            let p = load(weight_packed[wrb + pack_idx]);
            let s = load(scales[srb + g]).cast::<f32>();
            let b = load(biases[srb + g]).cast::<f32>();
            let dot = ((p >> 0u32) & 15u32).cast::<f32>() * s * x0
                ...
                + ((p >> 28u32) & 15u32).cast::<f32>() * s * x7
                + b * x7;
            let prev = stack_load("accs_hi", _m);
            stack_store("accs_hi", _m, prev + dot);
        }
    }
    // Cross-lane reduce + store for both banks.
    for _m in range(0u32, 8u32, 1u32) {
        let r = simd_sum(stack_load("accs_lo", _m));
        if lane == 0u32 {
            store(out[row * m_out + m_base + _m], r.cast::<T>());
        }
    }
    for _m in range(0u32, 8u32, 1u32) {
        let r = simd_sum(stack_load("accs_hi", _m));
        if lane == 0u32 {
            store(out[row * m_out + m_base + 8u32 + _m], r.cast::<T>());
        }
    }
}
...
pub fn mt_moe_gather_qmm_int4_m32<T>(
    ...
    let scale_expert_base = expert * m_out * groups_per_row;
    let x_row_base = row * k_in;
    // 32 accumulator slots in four 8-element banks.
    stack_alloc("accs_0", 8, "f32");
    stack_alloc("accs_8", 8, "f32");
    stack_alloc("accs_16", 8, "f32");
    stack_alloc("accs_24", 8, "f32");
    for _m in range(0u32, 8u32, 1u32) {
        stack_store("accs_0", _m, 0.0f32);
        stack_store("accs_8", _m, 0.0f32);
        stack_store("accs_16", _m, 0.0f32);
        stack_store("accs_24", _m, 0.0f32);
    }
    for pack_idx in range(lane, total_packs, 32u32) {
        let k_first = pack_idx * 8u32;
        let g = k_first / group_size;
        ...
        let x6 = load(x[x_row_base + k_first + 6u32]).cast::<f32>();
        let x7 = load(x[x_row_base + k_first + 7u32]).cast::<f32>();
        // m-cells 0..7 — DSL unrolls range(0,8) at codegen.
        for _m in range(0u32, 8u32, 1u32) {
            let wrb = weight_expert_base + (m_base + 0u32 + _m) * total_packs;
            let srb = scale_expert_base + (m_base + 0u32 + _m) * groups_per_row;
            let p = load(weight_packed[wrb + pack_idx]);
            let s = load(scales[srb + g]).cast::<f32>();
            let b = load(biases[srb + g]).cast::<f32>();
            let dot = ((p >> 0u32) & 15u32).cast::<f32>() * s * x0
                ...
                + ((p >> 28u32) & 15u32).cast::<f32>() * s * x7
                + b * x7;
            let prev = stack_load("accs_0", _m);
            stack_store("accs_0", _m, prev + dot);
        }
        // m-cells 8..15 — DSL unrolls range(0,8) at codegen.
        for _m in range(0u32, 8u32, 1u32) {
            let wrb = weight_expert_base + (m_base + 8u32 + _m) * total_packs;
            let srb = scale_expert_base + (m_base + 8u32 + _m) * groups_per_row;
            let p = load(weight_packed[wrb + pack_idx]);
            ...
            let dot = ((p >> 0u32) & 15u32).cast::<f32>() * s * x0
                ...
                + ((p >> 28u32) & 15u32).cast::<f32>() * s * x7
                + b * x7;
            let prev = stack_load("accs_8", _m);
            stack_store("accs_8", _m, prev + dot);
        }
        // m-cells 16..23 — DSL unrolls range(0,8) at codegen.
        for _m in range(0u32, 8u32, 1u32) {
            let wrb = weight_expert_base + (m_base + 16u32 + _m) * total_packs;
            let srb = scale_expert_base + (m_base + 16u32 + _m) * groups_per_row;
            let p = load(weight_packed[wrb + pack_idx]);
            let s = load(scales[srb + g]).cast::<f32>();
            let b = load(biases[srb + g]).cast::<f32>();
            let dot = ((p >> 0u32) & 15u32).cast::<f32>() * s * x0
                ...
                + ((p >> 28u32) & 15u32).cast::<f32>() * s * x7
                + b * x7;
            let prev = stack_load("accs_16", _m);
            stack_store("accs_16", _m, prev + dot);
        }
        // m-cells 24..31 — DSL unrolls range(0,8) at codegen.
        for _m in range(0u32, 8u32, 1u32) {
            let wrb = weight_expert_base + (m_base + 24u32 + _m) * total_packs;
            let srb = scale_expert_base + (m_base + 24u32 + _m) * groups_per_row;
            let p = load(weight_packed[wrb + pack_idx]);
            let s = load(scales[srb + g]).cast::<f32>();
            let b = load(biases[srb + g]).cast::<f32>();
            let dot = ((p >> 0u32) & 15u32).cast::<f32>() * s * x0
                ...
                + ((p >> 28u32) & 15u32).cast::<f32>() * s * x7
                + b * x7;
            let prev = stack_load("accs_24", _m);
            stack_store("accs_24", _m, prev + dot);
        }
    }
    // Cross-lane reduce + store for all four banks.
    for _m in range(0u32, 8u32, 1u32) {
        let r = simd_sum(stack_load("accs_0", _m));
        if lane == 0u32 {
            store(out[row * m_out + m_base + _m], r.cast::<T>());
        }
    }
    for _m in range(0u32, 8u32, 1u32) {
        let r = simd_sum(stack_load("accs_8", _m));
        if lane == 0u32 {
            store(out[row * m_out + m_base + 8u32 + _m], r.cast::<T>());
        }
    }
    for _m in range(0u32, 8u32, 1u32) {
        let r = simd_sum(stack_load("accs_16", _m));
        if lane == 0u32 {
            store(out[row * m_out + m_base + 16u32 + _m], r.cast::<T>());
        }
    }
    for _m in range(0u32, 8u32, 1u32) {
        let r = simd_sum(stack_load("accs_24", _m));
        if lane == 0u32 {
            store(out[row * m_out + m_base + 24u32 + _m], r.cast::<T>());
        }
    }
}

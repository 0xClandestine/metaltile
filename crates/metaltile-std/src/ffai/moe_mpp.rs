...
pub mod tests_support {
    ...
    fn make_shared_data(
        ...
        let x: Vec<f32> =
            (0..t_rows * k_in).map(|i| 0.05 * (i as f32 * 0.013).sin()).collect();
    ...
    #[test_kernel(name = "ffai/moe_mpp/bm16_vs_m1_f32", dtypes = [f32], tol = 1e-2)]
    fn test_moe_mpp_bm16_f32(dt: DType) -> TestSetup {
        ...
        let (x, weight_packed, scales, biases, indices, expert_offsets) =
            make_shared_data(n_experts, k_in, n_out, group_size, t_rows);
        ...
        let mut kernel = mt_moe_gather_qmm_mma_int4_bm16_mpp::kernel_ir_for(dt);
        ...
        TestSetup::new(kernel)
            ...
            .constexpr("k_in", k_in as u32)
            .constexpr("group_size", group_size as u32)
            .grid_3d((n_out / 32) as u32, t_rows.div_ceil(16) as u32, 1, [32, 1, 1])
            .compare_against(ref_setup)
    }

    #[test_kernel(name = "ffai/moe_mpp/bm16_vs_m1_bf16", dtypes = [bf16], tol = 1e-2)]
    fn test_moe_mpp_bm16_bf16(dt: DType) -> TestSetup {
        let n_experts = 4usize;
        let k_in = 64usize;
        let n_out = 64usize;
        let group_size = 32usize;

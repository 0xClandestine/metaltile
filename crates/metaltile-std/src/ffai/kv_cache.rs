...
use metaltile_core::DType;
    ...
    // This cannot be expressed as a single TestSetup, so helpers are preserved as dead code.
    // To run these tests, use the legacy `cargo test` integration test path.
}

...
mod tests_support {
    ...
    fn make_setup(n: usize, rows: usize, eps: f32, dt: DType) -> TestSetup {
        let tpg = n / 4;
        let x: Vec<f32> = ramp(rows * n, 23, 9.0)
            .into_iter()
            .map(|v| dt_round(v, dt))
            .collect();
        let residual: Vec<f32> = ramp(rows * n, 29, 7.0)
            .into_iter()
            .map(|v| dt_round(v, dt))
            .collect();

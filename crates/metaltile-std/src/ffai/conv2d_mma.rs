...
mod tests_support {
    ...
    fn small_vals(n: usize, modulus: usize, offset: f32, scale: f32) -> Vec<f32> {
        (0..n).map(|i| ((i % modulus) as f32 + offset) * scale).collect()
    }

    /// CPU reference for stride=1, dilation=1, pad=0 conv2d_mma.
    ...
    fn make_setup(
        ...
        TestSetup::new(kernel)
            .input(TestBuffer::from_vec("input", pack(&input_vals, dt), dt))
            .input(TestBuffer::from_vec("weight", pack(&weight_vals, dt), dt))
            ...
            .grid_3d(grid_x as u32, grid_y as u32, 1, [128, 1, 1])
    }

...
pub mod kernel_tests {
    ...
    /// Round-trip a slice through `dt` so the CPU oracle sees the same
    /// quantised values the kernel loads after load-cast. No-op for f32.
    ...
    // ── mt_sdpa_vector (d=128) ──────────────────────────────────────────────

    #[test_kernel(name = "mlx/sdpa_vector/d128/f32", dtypes = [f32], tol = 1e-3)]
    fn test_sdpa_vector_d128_f32(dt: DType) -> TestSetup {
        ...
        let mut kernel = mt_sdpa_vector::kernel_ir_for(dt);
    ...
    #[test_kernel(name = "mlx/sdpa_vector/d128/f16", dtypes = [f16], tol = 5e-3)]
    fn test_sdpa_vector_d128_f16(dt: DType) -> TestSetup {
        let head_dim = 128usize;
        let n_kv = 256usize;
        let n_q_heads = 4usize;
        let gqa_factor = 1usize;
        ...
        let q = round_through(&q_f32, DType::F16);
        let k = round_through(&k_f32, DType::F16);
        let v = round_through(&v_f32, DType::F16);
        let expected = cpu_sdpa_decode_reference(&q, &k, &v, n_q_heads, n_kv_heads, n_kv, head_dim);
        let mut kernel = mt_sdpa_vector::kernel_ir_for(dt);
        kernel.mode = KernelMode::Reduction;
        TestSetup::new(kernel)
    ...
    #[test_kernel(name = "mlx/sdpa_vector/d128/gqa4/f32", dtypes = [f32], tol = 1e-3)]
    fn test_sdpa_vector_d128_gqa4_f32(dt: DType) -> TestSetup {
        let head_dim = 128usize;
        let n_kv = 256usize;
    ...
    #[test_kernel(name = "mlx/sdpa_vector/d64/f32", dtypes = [f32], tol = 1e-3)]
    fn test_sdpa_vector_d64_f32(dt: DType) -> TestSetup {
        ...
        let v = ramp(n_kv_heads * n_kv * head_dim, 11, 5.0);
        let expected = cpu_sdpa_decode_reference(&q, &k, &v, n_q_heads, n_kv_heads, n_kv, head_dim);
        let mut kernel = mt_sdpa_vector_d64::kernel_ir_for(dt);
        kernel.mode = KernelMode::Reduction;
        TestSetup::new(kernel)
            .input(TestBuffer::from_vec("q", pack(&q, dt), dt))
            .input(TestBuffer::from_vec("k", pack(&k, dt), dt))
    ...
    #[test_kernel(name = "mlx/sdpa_vector/d64/f16", dtypes = [f16], tol = 5e-3)]
    fn test_sdpa_vector_d64_f16(dt: DType) -> TestSetup {
        let head_dim = 64usize;
        let n_kv = 256usize;
        let n_q_heads = 4usize;
        let gqa_factor = 1usize;
    ...
    #[test_kernel(name = "mlx/sdpa_vector/d64/gqa4/f32", dtypes = [f32], tol = 1e-3)]
    fn test_sdpa_vector_d64_gqa4_f32(dt: DType) -> TestSetup {
        let head_dim = 64usize;
        let n_kv = 256usize;
        let n_q_heads = 8usize;
        let gqa_factor = 4usize;
    ...
    #[test_kernel(name = "mlx/sdpa_vector/d96/f32", dtypes = [f32], tol = 1e-3)]
    fn test_sdpa_vector_d96_f32(dt: DType) -> TestSetup {
        ...
        let v = ramp(n_kv_heads * n_kv * head_dim, 11, 5.0);
        let expected = cpu_sdpa_decode_reference(&q, &k, &v, n_q_heads, n_kv_heads, n_kv, head_dim);
        let mut kernel = mt_sdpa_vector_d96::kernel_ir_for(dt);
        kernel.mode = KernelMode::Reduction;
        TestSetup::new(kernel)
            .input(TestBuffer::from_vec("q", pack(&q, dt), dt))
            .input(TestBuffer::from_vec("k", pack(&k, dt), dt))
    ...
    #[test_kernel(name = "mlx/sdpa_vector/d96/f16", dtypes = [f16], tol = 5e-3)]
    fn test_sdpa_vector_d96_f16(dt: DType) -> TestSetup {
        let head_dim = 96usize;
        let n_kv = 256usize;
        let n_q_heads = 4usize;
        let gqa_factor = 1usize;
    ...
    #[test_kernel(name = "mlx/sdpa_vector/d192/f32", dtypes = [f32], tol = 1e-3)]
    fn test_sdpa_vector_d192_f32(dt: DType) -> TestSetup {
        let head_dim = 192usize;
        let n_kv = 256usize;
        let n_q_heads = 4usize;
        let gqa_factor = 1usize;
        let n_kv_heads = n_q_heads / gqa_factor;
        ...
        let v = ramp(n_kv_heads * n_kv * head_dim, 11, 5.0);
        let expected = cpu_sdpa_decode_reference(&q, &k, &v, n_q_heads, n_kv_heads, n_kv, head_dim);
        let mut kernel = mt_sdpa_vector_d192::kernel_ir_for(dt);
        kernel.mode = KernelMode::Reduction;
        TestSetup::new(kernel)
            .input(TestBuffer::from_vec("q", pack(&q, dt), dt))
            .input(TestBuffer::from_vec("k", pack(&k, dt), dt))
    ...
    #[test_kernel(name = "mlx/sdpa_vector/d192/f16", dtypes = [f16], tol = 5e-3)]
    fn test_sdpa_vector_d192_f16(dt: DType) -> TestSetup {
        let head_dim = 192usize;
        let n_kv = 256usize;
        let n_q_heads = 4usize;
        let gqa_factor = 1usize;
    ...
    #[test_kernel(name = "mlx/sdpa_vector/d256/f32", dtypes = [f32], tol = 1e-3)]
    fn test_sdpa_vector_d256_f32(dt: DType) -> TestSetup {
        ...
        let v = ramp(n_kv_heads * n_kv * head_dim, 11, 5.0);
        let expected = cpu_sdpa_decode_reference(&q, &k, &v, n_q_heads, n_kv_heads, n_kv, head_dim);
        let mut kernel = mt_sdpa_vector_d256::kernel_ir_for(dt);
        kernel.mode = KernelMode::Reduction;
        TestSetup::new(kernel)
            .input(TestBuffer::from_vec("q", pack(&q, dt), dt))
            .input(TestBuffer::from_vec("k", pack(&k, dt), dt))
    ...
    #[test_kernel(name = "mlx/sdpa_vector/d256/f16", dtypes = [f16], tol = 5e-3)]
    fn test_sdpa_vector_d256_f16(dt: DType) -> TestSetup {
        let head_dim = 256usize;
        let n_kv = 256usize;
        let n_q_heads = 4usize;
        let gqa_factor = 1usize;
    ...
    #[test_kernel(name = "mlx/sdpa_vector/d256/gqa4/f32", dtypes = [f32], tol = 1e-3)]
    fn test_sdpa_vector_d256_gqa4_f32(dt: DType) -> TestSetup {
        let head_dim = 256usize;
        let n_kv = 256usize;
        let n_q_heads = 8usize;
        let gqa_factor = 4usize;

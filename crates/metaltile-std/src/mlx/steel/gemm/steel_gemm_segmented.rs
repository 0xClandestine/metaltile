...
mod tests_support {
    ...
    fn make_setup(dt: DType, total_k: usize, segments: Vec<u32>) -> TestSetup {
        let n_segments = segments.len() / 2;
        let a_raw = ramp(M * total_k, 19, 7.0);
        let b_raw = ramp(total_k * N, 23, 9.0);
        ...
        let mut kernel = mt_steel_gemm_segmented_64x64x16_2x2::kernel_ir_for(dt);
        ...
        TestSetup::new(kernel)
            ...
            .input(TestBuffer::from_vec("segments", pack_u32(&segments), DType::U32))
            .expect(TestBuffer::from_vec("out", pack(&expected, dt), dt))
            .constexpr("m", M as u32)
            .constexpr("n", N as u32)
            .constexpr("total_k", total_k as u32)
            .grid_3d((N / BN) as u32, (M / BM) as u32, n_segments as u32, [128, 1, 1])
    }

    #[test_kernel(name = "steel/gemm_segmented_single_full_f32", dtypes = [f32], tol = 2e-3)]
    fn test_gemm_segmented_single_full_f32(dt: DType) -> TestSetup {
        make_setup(dt, 48, vec![0u32, 48])
    }

    #[test_kernel(name = "steel/gemm_segmented_disjoint_f32", dtypes = [f32], tol = 2e-3)]
    fn test_gemm_segmented_disjoint_f32(dt: DType) -> TestSetup {
        make_setup(dt, 48, vec![0u32, 16, 16, 32, 32, 48])
    }

    #[test_kernel(name = "steel/gemm_segmented_uneven_f32", dtypes = [f32], tol = 2e-3)]
    fn test_gemm_segmented_uneven_f32(dt: DType) -> TestSetup {
        make_setup(dt, 48, vec![0u32, 16, 16, 48])
    }

    #[test_kernel(name = "steel/gemm_segmented_disjoint_f16", dtypes = [f16], tol = 8e-2)]
    fn test_gemm_segmented_disjoint_f16(dt: DType) -> TestSetup {
        make_setup(dt, 48, vec![0u32, 16, 16, 32, 32, 48])
    }

    #[test_kernel(name = "steel/gemm_segmented_uneven_bf16", dtypes = [bf16], tol = 5e-1)]
    fn test_gemm_segmented_uneven_bf16(dt: DType) -> TestSetup {
        make_setup(dt, 48, vec![0u32, 16, 16, 48])
    }
}

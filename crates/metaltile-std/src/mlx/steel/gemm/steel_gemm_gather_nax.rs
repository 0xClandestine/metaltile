...
use metaltile::test_kernel;
    ...
    fn make_setup(dt: DType, cos_tol_as_abs: f32) -> TestSetup {
        ...
        let expected = cpu_gather_gemm(&a, &b, &lhs, &rhs, m, n, k);
        let mut kernel = mt_steel_gemm_gather_nax::kernel_ir_for(dt);
        kernel.mode = KernelMode::Reduction;
        let obpe = dt.size_bytes();
        TestSetup::new(kernel)
            .input(TestBuffer::from_vec("a", pack(&a, dt), dt))
            .input(TestBuffer::from_vec("b", pack(&b, dt), dt))
            .input(TestBuffer::from_vec("lhs_indices", pack_u32(&lhs), DType::U32))
            .input(TestBuffer::from_vec("rhs_indices", pack_u32(&rhs), DType::U32))
            .expect(TestBuffer::from_vec("out", pack(&expected, dt), dt))
            .constexpr("k", k as u32)
            .constexpr("n", n as u32)
            .grid_2d((n / 32) as u32, (m / 32) as u32, [128, 1])
    }

    #[test_kernel(name = "steel/gemm_gather_nax_f32", dtypes = [f32], tol = 1e-3)]
    fn test_gemm_gather_nax_f32(dt: DType) -> TestSetup { make_setup(dt, 1e-3) }

    #[test_kernel(name = "steel/gemm_gather_nax_f16", dtypes = [f16], tol = 5e-2)]
    fn test_gemm_gather_nax_f16(dt: DType) -> TestSetup { make_setup(dt, 5e-2) }

    #[test_kernel(name = "steel/gemm_gather_nax_bf16", dtypes = [bf16], tol = 2e-1)]
    fn test_gemm_gather_nax_bf16(dt: DType) -> TestSetup { make_setup(dt, 2e-1) }
}

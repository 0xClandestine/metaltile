//! `mt_qmm_nax` — int4 quantized matmul via `mpp::tensor_ops::matmul2d`.
//!
//! NAX (Apple tensor-core) port of `affine_qmm_t_nax` from MLX
//! `metal/kernels/quantized_nax.metal`. Gated behind the `nax` Cargo
//! feature — the kernel requires the Metal 4 `MetalPerformancePrimitives`
//! framework (macOS 26+) and the codegen emits the framework include
//! when it detects the `mpp::` marker in the `Op::InlineMsl` body.
//!
...
//! `crates/metaltile-std/tests/quantized_nax_gpu_correctness.rs`.
...
/// default).
const QMM_MMA_MPP_SRC_TEMPLATE: &str = r#"// --- mt_qmm_nax body (BM=BN=BK=32, TG=128, 4 SGs WM=WN=2) ---
...
/// Build the per-dtype [`Kernel`] IR for `mt_qmm_nax_{T}`.
///

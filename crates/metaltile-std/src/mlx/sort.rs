...
mod tests_support {
    ...
    // ── mt_sort (single-block bitonic) ───────────────────────────────────

    #[test_kernel(name = "mlx/sort_f32", dtypes = [f32], tol = 1e-6)]
    fn test_sort_single_block_f32(dt: DType) -> TestSetup {
        const N: usize = 1024;
    ...
    // ── mt_sort_segmented (per-row bitonic) ──────────────────────────────

    #[test_kernel(name = "mlx/sort_segmented_n1024_f32", dtypes = [f32], tol = 1e-6)]
    fn test_sort_segmented_n1024_f32(dt: DType) -> TestSetup {
        let (n, batch) = (1024usize, 1usize);
        let inp: Vec<f32> = (0..n).rev().map(|i| i as f32 * 0.1).collect();
    ...
    #[test_kernel(name = "mlx/sort_segmented_n512_f32", dtypes = [f32], tol = 1e-6)]
    fn test_sort_segmented_n512_f32(dt: DType) -> TestSetup {
        let (n, batch) = (512usize, 2usize);

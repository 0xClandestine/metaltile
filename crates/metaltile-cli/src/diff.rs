...
//! Standalone diff engine — compares baseline vs current bench results.
//!
//! Extracted from `cmd::diff` so both `tile bench` (auto-diff) and
//! `tile diff` can use it without circular coupling.
...
/// Compare baseline and current result arrays, print a diff table, and
...
fn sort_diff_rows(rows: &mut [DiffRow], sort: &str) {
    match sort {
        "delta" => rows.sort_by(|a, b| {
            b.delta_pct
                .unwrap_or(0.0)
                .partial_cmp(&a.delta_pct.unwrap_or(0.0))
                .unwrap_or(std::cmp::Ordering::Equal)
        }),
        "regression" => rows.sort_by(|a, b| {
